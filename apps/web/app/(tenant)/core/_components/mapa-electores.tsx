...
import { geocodificarPendientes, type VoterGeo, type GeoStats, type StationGeo } from '../actions'
...
export function MapaElectores({ puntos, geoStats, puestos }: { puntos: VoterGeo[]; geoStats: GeoStats; puestos: StationGeo[] }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      else                        dibujarCapaPuestos(L, capa, puestos)
      capa.addTo(mapa)
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
        {(['residencia', 'puesto'] as const).map((v) => (
          <button
            key={v}
            onClick={() => setVista(v)}
            style={{
          ...
          >
            {v === 'residencia' ? 'Por residencia' : 'Por puesto de votación'}
          </button>
        ))}
      </div>

      {vista === 'residencia'
        ? <ControlesResidencia puntos={puntos} geoStats={geoStats} msg={msg} isPending={isPending} onUbicar={ubicar} />
        : <ControlesPuesto puestos={puestos} />}
